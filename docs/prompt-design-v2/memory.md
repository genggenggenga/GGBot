...
源码：`core/prompts/memory.py`

### 7.1 用户画像提取

职责：提取跨会话仍有效、能改善后续服务的明确偏好。
...
- 只有 `role=user` 的明确陈述可作为证据；
- 客服回复不能反向成为用户偏好；
- “这次先发短信”不自动等于长期沟通偏好；
- 没有稳定偏好时返回空结果。

运行时通过 `_UserProfileOutput` 强制调用 `submit_user_profile`。除 Prompt 示例中的 `preferences`、`language` 和 `communication_preference` 外，Schema 还允许 `contact_preference`、`timezone` 与 `accessibility`。存储前仍会经过字段白名单和时效事实过滤，结构化输出不能绕过本地治理。

### 7.2 工作记忆压缩

职责：压缩同一会话早期消息，使状态机能够继续当前任务。
...
重点：服务于“继续当前对话”，不能丢失未完成状态。

### 7.3 情景记忆记录

职责：在任务完成或转人工时记录跨会话可复用的历史处理事件。
...
| 服务当前会话续接 | 服务未来会话回忆 |
| 重点保留未完成状态 | 重点记录最终结果 |
| 可以包含待确认动作 | 说明动作最终是否完成 |
| 持续覆盖更新 | 事件触发写入 |

重点：区分申请、审核、到账等业务阶段，不把单次情绪写成长期画像。

情景摘要仍是纯文本生成，不使用结构化 Tool Calling。任务完成或转人工后，API 在同会话 Redis 锁内同步生成并写入事件，避免摘要读取到下一轮消息；代价是摘要和 Chroma 延迟会增加本轮锁持有时间。
...
```text
你是 GGBot 的客服用户画像提取器。你的职责是从用户本人明确表达中提取有助于后续服务、跨会话仍有效的偏好；不是总结对话，也不记录业务事件。
...
从以下对话数据提取长期偏好：
[
  {"role": "user|assistant", "content": "{对话内容}"}
]
```
### 工作记忆压缩
...
```text
你是 GGBot 的工作记忆压缩器。你的职责是让同一客服会话在截断早期消息后仍能无损继续处理当前任务。
...
```json
{
  "summary_kind": "working_memory",
  "summary_requirement": "{摘要长度与内容要求}",
  "conversation": "{旧摘要与待压缩对话}"
}
```
### 情景记忆记录
...
```text
你是 GGBot 的客服情景记忆记录器。你的职责是在任务完成或转人工时，记录可供未来客服理解历史处理结果的事件摘要，不负责延续当前状态机。
