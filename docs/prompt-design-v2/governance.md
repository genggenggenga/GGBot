...
| Grounded Answer | `build_answer_generation_prompt` | `rag/answer_generator.py` |
| Query Rewrite | `build_query_rewrite_prompt` | `mcp/tool_manager.py` |
| Rerank | `build_rerank_prompt` | `mcp/tool_manager.py` |
| 回答润色 | `build_prompt` | `core/response_polisher.py` |
...
主运行时的 NLU、ReAct、Query Planner、Grounded Answer、回答润色和画像提取共享 `core/structured_llm.py`。旧 Query Rewrite、Rerank、Legacy 和 LLM Judge 仍使用严格 JSON 文本输出，只用于兼容或评测路径。
...
- [ ] 是否明确输出 Schema 和枚举；
- [ ] Pydantic Schema 与 Prompt 示例是否一致；
- [ ] 强制 Tool Calling 的 tool name 是否稳定且唯一；
- [ ] Tool Calling 失败时是否有 fail closed 或可证明安全的 fallback；
- [ ] 是否对缺失信息给出安全降级；
