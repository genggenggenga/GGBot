...
运行时通过 `_GeneratedAnswer` Schema 强制调用 `submit_grounded_answer`。模型返回充分证据时，本地继续检查逐段引用、引用集合，以及回答是否新增证据和问题中不存在的数字、期限、金额、ID 或错误码。证据不足时明确拒答；Tool Calling、超时或校验失败时降级为 Top-1 抽取式回答。
