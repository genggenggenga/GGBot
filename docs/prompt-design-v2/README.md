...
flowchart LR
    ...
    AfterSales --> Skill[按 Agent + Intent 匹配的售后 Skill]
    QueryPlanner --> Retrieval[混合检索与重排]
    Retrieval --> AnswerGenerator[Grounded Answer Generator]
    AnswerGenerator --> Polisher[受约束回答润色]
    ReAct --> Polisher
...
机器消费的主链路 Prompt 通过 `StructuredLLMClient` 执行。运行时使用 Anthropic `tools` 和强制 `tool_choice`，让模型把结构化结果放入指定 tool input，再由 Pydantic 校验：

```text
PromptSpec
  -> StructuredLLMClient.generate(output_model, tool_name)
  -> forced tool_use
  -> tool input
...
JSON 文本契约仍保留在 Prompt 中用于约束语义，但运行时不再依赖 Markdown 清理或首尾大括号截取。
...
- 通过 Pydantic 模型生成 JSON Schema；
- 使用稳定且唯一的强制 tool name；
- 禁止 Markdown 和额外解释；
- 使用固定枚举；
- 对缺失信息使用空值或指定降级值；
- 对置信度给出统一校准标准。

结构化输出成功不等于业务结果可信。NLU 仍校验意图与原文槽位，ReAct 仍校验工具白名单和确认状态，RAG Answer 仍校验引用及受保护数字。
